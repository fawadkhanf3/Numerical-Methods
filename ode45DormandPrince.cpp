...
double **ode45DormandPrince(double *tspan, double *xinit, int &length, int &successful, int &failed);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double **yp = ode45DormandPrince(Tspan, y0, length, successful_steps, failed_steps);
...
double **ode45DormandPrince(double *tspan, double *xinit, int &length, int &successful, int &failed) {
	...
	const double A[6] = { 1.0 / 5.0,3.0 / 10.0,4.0 / 5.0,8.0 / 9.0,1.0,1.0 },
		B[7][6] = { {1.0 / 5.0,3.0 / 40.0,44.0 / 45.0,19372.0 / 6561.0,9017.0 / 3168.0,35.0 / 384.0},
		...
		Bi[7] = { 35.0 / 384.0,0.0,500.0 / 1113.0,125.0 / 192.0,-2187.0 / 6784.0,11.0 / 84.0,0.0 },
		Bi_star[7] = { 5179.0 / 57600.0,0.0,7571.0 / 16695.0,393.0 / 640.0,-92097.0 / 339200.0,187.0 / 2100.0,1.0 / 40.0 };

	double E[7];

	const double atol = 1.0e-6, rtol = 1.0e-6, pwr = 1.0 / 5.0;
	double hmax = (tspan[1] - tspan[0]) / 2.5, hmin = (tspan[1] - tspan[0]) / 1e20;
	double h = (tspan[1] - tspan[0]) / 100.0, temp[6], temp2[6];
	double tsave[500], ysave[6][500], t, y[nvar], e[6], delta, scale, err;
	int icount = 0, rejected = 0, accepted = 1;
	...
	while (Alt >= ALT_T && h >= hmin) {
		if (t + h > tspan[1]) h = tspan[1] - t;
		for (int k = 0; k < 6; k++) {
			hA[k] = 0.0;
			for (int m = 0; m < 7; m++) {
				hB[m][k] = 0.0;
		...
		for (int f = 1; f < 7; f++) {
			...
			for (int k = 0; k < 6; k++) {
				temp[k] = 0.0;
				for (int g = 0; g < 7; g++) {
					temp[k] += K[k][g] * hB[g][f - 1];
				}
				temp[k] += y[k];
			}

			dots = eoms(t + hA[f - 1], temp);
		...
		normxn = sqrt(normxn);
		delta = h*sqrt(delta);
		scale = MAXI(rtol*MAXI(normx, normxn), atol);
		...
		h = MINI(hmax, 0.8*h*pow(1 / err, pwr));
	}
