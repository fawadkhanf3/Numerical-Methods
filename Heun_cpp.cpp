...
double **Heun(double *tspan, double *xinit, int &length);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int length;
	double **yp = Heun(Tspan, y0, length);
...
double **Heun(double *tspan, double *xinit, int &length) {
	...
	while (Alt >= ALT_T) {
		...
		// k2 = f(t+h,x+k1*h)
		for (int k = 0; k < nvar; k++) {temp[k] = y[k] + h*K[0][k];}
		...
		for (int k = 0; k < nvar; k++) {
			temp[k] = y[k] + 1.0/2.0*h*(K[0][k] + K[1][k]);
		}
